...
export async function updateExpense(expenseData) {
  try {
    if (!isSupabaseConfigured()) {
      console.warn('⚠️ Supabase non configurato, uso demo storage per aggiornamento spesa')
      const { updateDemoExpense } = await import('./demo-storage')
      return updateDemoExpense(expenseData)
    }
    ...
    return { id: expenseData.id, ...expenseData }
  }
}
...
export async function updateIncome(incomeData) {
  try {
    if (!isSupabaseConfigured()) {
      ...
      return updateDemoIncome(incomeData)
    }
