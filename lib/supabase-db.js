...
export async function createMainCategory(categoryData) {
  try {
    if (!isSupabaseConfigured()) {
      ...
      return addDemoCategory(categoryData.name)
    }
...
export async function createSubcategory(subcategoryData) {
  try {
    if (!isSupabaseConfigured()) {
      ...
      return addDemoSubcategory(subcategoryData.name, subcategoryData.mainCategoryId)
    }
...
export async function getExpenses(month = null) {
  try {
    ...
    return (data || []).map(expense => ({
      ...
      paymentMethod: expense.payment_method || 'contanti',
  ...
  try {
    ...
    const { data, error } = await supabase
      ...
      .insert([{
        ...
        payment_method: expenseData.paymentMethod || 'contanti'
      }])
      .select()
      .single()
...
export async function getIncomes(month = null) {
  try {
    ...
    return (data || []).map(income => ({
      ...
      paymentMethod: income.payment_method || 'contanti',
  ...
  try {
    ...
    const { data, error } = await supabase
      ...
      .insert([{
        ...
        payment_method: incomeData.paymentMethod || 'contanti'
      }])
      .select()
      .single()
