...
export async function deleteSubcategory(params) {
  try {
    ...
    if (typeof params === 'string' || typeof params === 'number') {
      ...
      // Trova la sottocategoria per ottenere il main_category_id
      ...
      if (subcategoryError) {
        console.error('❌ Errore nella ricerca della sottocategoria per ID:', subcategoryError)
        throw new Error(`Errore nella ricerca della sottocategoria: ${subcategoryError.message}`)
      }
      
      if (!subcategory) {
        throw new Error(`Sottocategoria con ID ${subcategoryId} non trovata`)
      }
      ...
      if (mainCategoryError) {
        console.error('❌ Errore nella ricerca della categoria principale:', mainCategoryError)
        throw new Error(`Errore nella ricerca della categoria principale: ${mainCategoryError.message}`)
      }
      ...
      // Trova la sottocategoria (usa maybeSingle per evitare errore se non trovata)
      const { data: subcategory, error: subcategoryError } = await supabase
        .from('subcategories')
        .select('id')
      ...
      if (subcategoryError) {
        console.error('❌ Errore nella ricerca della sottocategoria:', subcategoryError)
        throw new Error(`Errore nella ricerca della sottocategoria: ${subcategoryError.message}`)
      }
      
      if (!subcategory) {
        throw new Error(`Sottocategoria "${params.subcategoryName}" non trovata nella categoria "${params.mainCategoryName}"`)
      }
      
      subcategoryId = subcategory.id
    }
    
    // Controlla se la sottocategoria è usata in spese o entrate
    const { data: expensesUsingSubcategory } = await supabase
      .from('expenses')
      .select('id')
    ...
    if (expensesUsingSubcategory && expensesUsingSubcategory.length > 0) {
      throw new Error('Impossibile eliminare: la sottocategoria è usata da alcune spese')
    }
    
    const { data: incomesUsingSubcategory } = await supabase
      .from('incomes')
      .select('id')
      .eq('subcategory_id', subcategoryId)
      .limit(1)
    
    if (incomesUsingSubcategory && incomesUsingSubcategory.length > 0) {
      throw new Error('Impossibile eliminare: la sottocategoria è usata da alcune entrate')
    }
    
    // Ora cancella la sottocategoria
    const { error } = await supabase
      .from('subcategories')
      .delete()
      .eq('id', subcategoryId)
    ...
    console.log('✅ Sottocategoria eliminata con successo:', subcategoryId)
    return true
  } catch (error) {
    console.error('❌ Errore in deleteSubcategory:', error)
    throw error // Non fare fallback a demo-storage, propaga l'errore
  }
}
...
export async function updateDeadline(id, updates) {
  try {
    ...
    // Converti 'paid' in 'is_paid' per il database
    const dbUpdates = { ...updates }
    if ('paid' in dbUpdates) {
      dbUpdates.is_paid = Boolean(dbUpdates.paid)
      delete dbUpdates.paid
    }
    
    console.log('🔧 updateDeadline chiamata con:', { deadlineId, updates, dbUpdates })
    ...
    // Converti 'is_paid' in 'paid' per il frontend
    const result = {
      ...data,
      paid: Boolean(data.is_paid)
    }
